...
import { ArrowLeft, Save, Upload, X, FileText, Image, Trash2, Download } from "lucide-react";
      ...
      if (client.ndis_plan_document_id) {
        ...
        if (!docError && document) {
          setExistingNdisDocument({
            ...
            size: 0, // We don't have size info in database
          });
        }
      }
        ...
        <div className="header flex flex-col sm:flex-row justify-between items-start sm:items-center px-3 py-1 border-b mb-6 border-gray-400 gap-4">
          <div className="flex items-center">
            <Link to={'/clients'} className="flex items-center hover:text-blue-600 transition-colors">
              <ArrowLeft size={20} className="mr-1" /> Back to Clients
            </Link>
          </div>
          
          <h2 className="text-2xl font-semibold text-slate-800 text-center">
            Edit Client: {clientData.first_name} {clientData.last_name}
          </h2>
          ...
          <div className="border border-slate-200 rounded-xl p-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-sm text-slate-500 mt-1">Set client status</p>
              </div>
              <div className="flex items-center gap-3">
                <span className="text-sm text-slate-600">{clientData.is_active ? 'Active' : 'Inactive'}</span>
